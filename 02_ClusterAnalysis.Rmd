...
ggplot(smoothed %>% filter(Hylak_id %in% vis$Hylak_id)) + 
  ...
  geom_line(aes(x = doy, y = smoothed), color = 'red', size = 1) + 
...
clusters.long <- c.norm %>% pivot_longer("1":"365", names_to = 'Week', values_to = 'norm_dWL') %>% as.data.table()
...
iqrs.norm <- clusters.long[,.(iqr75 = quantile(norm_dWL, .75), 
                         ...
                         med_dWL = median(norm_dWL)), by = .(cluster, Week)
                         ][,Week := as.numeric(Week)]

p2 <- ggplot(iqrs.norm, aes(x = Week)) + 
  geom_ribbon(aes(ymin = iqr25, ymax = iqr75), fill = 'grey70') +
  geom_line(aes(y = med_dWL)) +
  facet_wrap(~cluster, ncol = 1) +
...
check <- links.base %>% pivot_longer(`(1984,1990]`:`(2014,2020]`, names_to = 'Period', values_to = 'Cluster') %>% group_by(Hylak_id) %>% summarize(States = factor(length(unique(Cluster)), levels = c(1:3), labels = c(1:3)))
...
p1 <- ggplot(links.sf, aes(x = States, y = Vol_total)) + geom_boxplot() + scale_y_continuous(trans = 'log10') + labs(x = 'Stability') + coord_cartesian(ylim = c(.01,100))

p2 <- ggplot(links.sf, aes(x = States, y = Res_time)) + geom_boxplot() + scale_y_continuous(trans = 'log10') + labs(x = 'Stability') + coord_cartesian(ylim = c(1,1000))

p3 <- ggplot(links.sf, aes(x = States, y = Slope_100)) + geom_boxplot() + scale_y_continuous(trans = 'log10') + labs(x = 'Stability') + coord_cartesian(ylim = c(.1,10))

p4 <- ggplot(links.sf, aes(x = States, y = Depth_avg)) + geom_boxplot() + scale_y_continuous(trans = 'log10') + labs(x = 'Stability') + coord_cartesian(ylim = c(1,10))
...
Modes <- function(x) {
  ...
  mode <- ux[tab == max(tab)]
  mode <- ifelse(length(mode) > 1, 'mixed', mode)
...
grid <- grid %>% st_join(clusters.sf, left = F) %>% 
  group_by(ID, period) %>%
  mutate(modal_clust <- getmode(cluster))
