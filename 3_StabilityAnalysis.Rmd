...
stability.long <- stability %>% group_by(totalChange) %>%
  summarise_at(vars(Lake_area:Wshd_area, lwRatio, total_precip_mm_mean:spRatio_cv), median, na.rm = T)
...
"Lake_area"         , "Shore_len"            , "Shore_dev"                ,
"Vol_total"         , "Vol_res"              , "Vol_src"                  ,
"Depth_avg"         , "Dis_avg"              , "Res_time"                 ,
"Elevation"         , "Slope_100"            , "Wshd_area"                ,
"lwRatio"           , "total_precip_mm_mean" , "mean_annual_temp_k_mean"  ,
"pop_sum_mean"      , "seasonal_km2_mean"    , "permanent_km2_mean"       ,
"total_km2_mean"    , "spRatio_mean"         , "mean_monthly_precip_mm_cv",
"total_precip_mm_cv", "mean_annual_temp_k_cv", "pop_sum_cv"               ,
"seasonal_km2_cv"   , "permanent_km2_cv"     , "total_km2_cv"             ,
"spRatio_cv"),
namesNew = c("Lake_area"          , "Shore Length (km)"        , "Shore Development"        ,
"Volume (cu. km)"      , "Vol_res"                  , "Vol_src"                  ,
"Depth (m)"            , "Discharge (cu. m/s)"      , "Residence Time (days)"    ,
"Elevation (m)"        , "Surrounding Slope"        , "Watershed Area (sq. km)"  ,
"Lake/Watershed Ratio" , "Mean Annual Precip (mm)"  , "Mean Temperature (k)"     ,
"Population"           , "Seasonal Water (sq. km)"  , "Permanent Waters (sq. km)",
"Total Area (sq. km)"  , "Seasonal/Permanent Ratio" , "CV of Monthly Precip"     ,
"CV of Annual Precip"  , "CV of Mean Annual Temp"        , "CV of Population"         ,
"CV of Seasonal Waters", "CV of Permanent Water"    , "CV of Total Area"         ,
"CV of Seasonal/Permanent Ratio"))
...
corPlot <- stability %>%
  ...
  select(-c(Hylak_id, Lake_area, Vol_src, Vol_res)) %>%
...
ggsave('figures/CorPlot.png', plot = corPlot, type = 'windows', width = 6.5, heigh = 6.5, units = 'in')

lms <- lms %>% filter(term == 'value') %>% left_join(renamer)

stab.sum <- stability %>%
  select(totalChange,Lake_area:Wshd_area, lwRatio, total_precip_mm_mean:spRatio_cv) %>%
...
lms %>% ungroup() %>%
  ...
  # kableExtra::as_image(width = 8, file = 'figures/lmTable.png')
