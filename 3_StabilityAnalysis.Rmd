...
# See if we can predict clusters for super stable lakes

```{r}
stable.ids <-  stability %>% filter(States == 1)

stable.lakes <- clusters.sf %>% st_set_geometry(NULL) %>%
  filter(Hylak_id %in% stable.ids$Hylak_id) %>%
  select(-c(period,lpID)) %>%
  distinct() %>%
  left_join(glcp.avg) %>%
  mutate(uniqueID = c(1:nrow(.))) %>%
  select(-c(Lake_name:Grand_id, Pour_long, Pour_lat, Hylak_id)) %>%
  na.omit()

library(randomForest)

train <- stable.lakes %>% group_by(cluster) %>% sample_frac(.7)
test <- stable.lakes %>% filter(!uniqueID %in% train$uniqueID)
train <- train %>% select(-uniqueID)
rf = randomForest(cluster ~ ., data=train, ntree=100, mtry=2, importance=TRUE)
rf
varImpPlot(rf)
prediction_for_table <- predict(rf, test %>% select(-uniqueID))
caret::confusionMatrix(prediction_for_table, test$cluster)
