...
vis <- smoothed.raw %>% filter(Hylak_id %in% c(8736, 9086, 69, 792, 9092)) %>%
  pivot_longer(-c(Hylak_id, period), names_to = 'doy', values_to = 'dWL') %>%
...
vis <- smoothed.norm %>% filter(Hylak_id %in% c(8736, 9086, 69, 792, 9092)) %>%
  pivot_longer(-c(Hylak_id, period), names_to = 'doy', values_to = 'dWL') %>%
  mutate(doy = as.numeric(doy)) %>%
  left_join(vis.samp)
...
ggplot(vis, aes(x = doy)) + 
  ...
  labs(x = 'Day of Year', y = 'Z-Normalized Dominant Wavelength') +
  facet_grid(LakeName~period, scales = 'free', labeller = labeller(LakeName = label_wrap_gen(10)))
