...
## Now do the same for just for May-October
summerCounts <- srCor[biWeek >= 18 & biWeek <= 42][, .N, by = .(Hylak_id, period, biWeek)]

# Find all possible combos to make sure its exhaustive
combos <- as.data.table(expand.grid(biWeek = unique(summerCounts$biWeek), Hylak_id = unique(srCor$Hylak_id), period = unique(summerCounts$period)))

summerFilter <- merge(summerCounts,combos, all = T)[is.na(N)]


lakeSamp <- srCor[biWeek >= 18 & biWeek <= 42 & Hylak_id %in% unique(Filter$Hylak_id)
                  ][!Hylak_id %in% unique(summerFilter$Hylak_id),
                  .(Hylak_id, period, date, dWL, doy)
                  ][,dWL := scale(dWL), by = .(Hylak_id, period)]

length(unique(lakeSamp$Hylak_id))
check <- lakeSamp %>% group_by(Hylak_id, period) %>% summarize(count = n(), mean = mean(dWL), sd = sd(dWL))
...
min(lakeSamp$doy)
max(lakeSamp$doy)
k.smoother.summer <- function(data){
...
tic()
test <- smoothed[,kmeans := map(data, k.smoother.summer)
                       ][, .(Hylak_id, period, kmeans)
                         ][,unnest(.SD, kmeans)]
toc()
...
test <- smoothed %>%
  as_tibble() %>%
  mutate(kmeans = future_map(data, k.smoother.summer, .progress = T)) %>%
  select(-data) %>%
  unnest(kmeans)
plan(sequential)
toc()

check <- lakeSamp %>% group_by(period, Hylak_id) %>% summarise(count = n())

vis <- test %>% distinct(Hylak_id) %>% sample_n(10)

ggplot(test %>% filter(Hylak_id %in% vis$Hylak_id)) + 
